...
def plot_vol(prices_train):
    ...
    # print(nan_count)
    diff_vol_years = (
        volatility[TRADING_DAYS : TRADING_DAYS + TRADING_DAYS]
        - volatility[TRADING_DAYS + TRADING_DAYS : len(volatility)]
    )
...
def get_low_medium_high_price(df, low_perc=0.2, medium_perc=0.7):
    ...
    low, medium, high = (
        df[0 : int((low_perc * len(df)))],
        df[int((low_perc * len(df))) : int(((medium_perc + low_perc) * len(df)))],
        df[int((medium_perc + low_perc) * len(df)) :],
    )
